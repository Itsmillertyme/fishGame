Shader "Hidden/Locked/.poiyomi/Poiyomi Toon URP/429b1e049acbd6448807eb6e2d9535ca"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void PoiCalculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				if (float4(0.003676505,0.07818741,0.01117768,0.8666667).a > 0)
				{
					#if defined(PROP_SHADOW2NDCOLORTEX) || !defined(OPTIMIZER_ENABLED)
					...
					#endif
					shadow2ndColorTex.rgb = lerp(float3(1, 1, 1), shadow2ndColorTex.rgb, shadow2ndColorTex.a) * float4(0.003676505,0.07818741,0.01117768,0.8666667).rgb;
					lns.y = float4(0.003676505,0.07818741,0.01117768,0.8666667).a - lns.y * float4(0.003676505,0.07818741,0.01117768,0.8666667).a;
			...
			float4 frag(VertexOut i, bool facing : SV_IsFrontFace) : SV_Target
			{
				...
				poiFragData.baseColor *= poiThemeColor(poiMods, float4(0.02315336,0.05126945,0.02732088,1).rgb, 0.0);
				poiFragData.alpha = mainTexture.a * float4(0.02315336,0.05126945,0.02732088,1).a;
				#else
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyOutlineColor(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods, in PoiCam poiCam)
			{
				...
				float4 outlineColor = float4(0.05271218,0.6562665,0.1088418,1);
				#ifdef POI_AUDIOLINK
