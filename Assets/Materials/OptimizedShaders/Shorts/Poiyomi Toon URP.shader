Shader "Hidden/Locked/.poiyomi/Poiyomi Toon URP/c3360fdaec454b84e91af6b0ef5d6d79"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void PoiCalculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				if (float4(0.07818741,0.05849556,0.003676505,0.8666667).a > 0)
				{
					#if defined(PROP_SHADOW2NDCOLORTEX) || !defined(OPTIMIZER_ENABLED)
					...
					#endif
					shadow2ndColorTex.rgb = lerp(float3(1, 1, 1), shadow2ndColorTex.rgb, shadow2ndColorTex.a) * float4(0.07818741,0.05849556,0.003676505,0.8666667).rgb;
					lns.y = float4(0.07818741,0.05849556,0.003676505,0.8666667).a - lns.y * float4(0.07818741,0.05849556,0.003676505,0.8666667).a;
			...
			float4 frag(VertexOut i, bool facing : SV_IsFrontFace) : SV_Target
			{
				...
				poiFragData.baseColor *= poiThemeColor(poiMods, float4(0.2961383,0.2501584,0.1529262,1).rgb, 0.0);
				poiFragData.alpha = mainTexture.a * float4(0.2961383,0.2501584,0.1529262,1).a;
				#else
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyOutlineColor(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods, in PoiCam poiCam)
			{
				...
				float4 outlineColor = float4(0.272771,0.195662,0.1068815,1);
				#ifdef POI_AUDIOLINK
